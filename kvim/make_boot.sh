...
cp $KERNEL_OUT/arch/arm64/boot/Image.gz out/target/product/kvim/kernel

if [ "$2" != "m" ]; then
    make -C common O=../$KERNEL_OUT kvim.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    #cp -f $KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxl_kvim_2g.dtb out/target/product/kvim/dtb.img
    $DTBTOOL -o out/target/product/kvim/dtb.img -p $KERNEL_OUT/scripts/dtc/ $KERNEL_OUT/arch/arm64/boot/dts/amlogic/
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image.gz \
        ...
        --ramdisk ${ROOTFS} \
        --second out/target/product/kvim/dtb.img \
